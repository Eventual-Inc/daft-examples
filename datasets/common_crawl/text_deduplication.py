...
def get_band_idx(bands: list[list[int]], B: int) -> list[int]:
    """Return band indices for exploded bands."""
    if bands is None:
        return []
...
def _canonicalize_edges(edges: DataFrame) -> DataFrame:
    """Order edges so u < v and deduplicate."""
    return (
...
def _edge_sets_equal(a: DataFrame, b: DataFrame) -> bool:
    """Check edge set equality after canonicalization."""
...
def _pairs_equal(a: DataFrame, b: DataFrame) -> bool:
    """Check equality for (u, rep) pairs."""
...
def connected_components(edges: DataFrame) -> DataFrame:
    ...
    nodes = b_final.select(col("u")).union_all(b_final.select(col("v").alias("u"))).distinct()
...
if __name__ == "__main__":
    ...
    # 6) Keep only representatives (deduped)
