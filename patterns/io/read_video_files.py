...
def read_video_meta(
    ...
    *,
    ...
    with file.to_tempfile() as tmp:
        with av.open(
            tmp.name, mode="r", options=options, metadata_encoding="utf-8"
        ) as container:
            video = next(
    ...
    with file.to_tempfile() as tmp:
        with av.open(
            ...
            for frame in container.decode(vs):
                ...
                payload = image_array.astype(np.uint8, copy=False).tobytes()
    ...
    try:
        with file.to_tempfile() as tmp:
            with av.open(
                ...
                if stream is None:
                    raise ValueError(f"No audio stream found in {file!r}")
        ...
        return {"audio_array": np.zeros((0,), dtype=np.float32), "sample_rate": int(target_sr)}

    audio = np.concatenate(chunks, axis=0).astype(np.float32, copy=False) if chunks else np.zeros((0,), dtype=np.float32)
    return {"audio_array": audio, "sample_rate": int(target_sr)}
