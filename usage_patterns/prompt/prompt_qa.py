...
df = df.with_column(
    "question",
    prompt(
        format("Generate an orthogonal out of band question about {} ", daft.col("topic")),
        model="gpt-5-nano",
    ...
    "judgement",
    prompt(
        format("Is the answer to the question correct? Use your tools to verify the answer. <question>{}</question>  <answer>{}</answer>", daft.col("question"), daft.col("answer")),
        model="gpt-5.1",
        reasoning={"effort": "high"},
    )
)
